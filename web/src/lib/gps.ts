/**
 * Filtre GPS — **fichier miroir de `mobile/src/lib/gps.ts`**.
 *
 * Les deux plateformes appliquent EXACTEMENT le même filtre, et c'est capital :
 * un point accepté sur mobile mais rejeté sur le web produirait deux distances
 * différentes pour la même sortie. Le serveur recalcule tout à la finalisation
 * (règle « le client n'est jamais cru »), mais l'affichage en direct, lui,
 * vient du client — et il doit annoncer les mêmes chiffres.
 *
 * Toute correction ici doit être reportée à l'identique côté mobile.
 */
